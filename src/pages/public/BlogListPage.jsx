...
export default function BlogListPage() {
  ...
  useEffect(() => { dispatch(fetchPublicCategories()); dispatch(fetchPublicTags()); }, [dispatch]);
        ...
        <aside className="w-full shrink-0 md:w-56">
          <div className="relative mb-4">
            <input
              ...
              onKeyDown={(e) => { if (e.key === 'Enter') updateParams({ search: searchInput }); }}
              ...
              <li>
                <button
                  ...
                  className={`w-full rounded-lg px-3 py-1.5 text-left text-sm transition-colors ${
                    !category ? 'bg-muted font-medium text-primary' : 'text-muted-foreground hover:bg-muted hover:text-foreground'
                  }`}
                >
                  All {categories.length > 0 && `(${categories.reduce((sum, c) => sum + (c.posts_count || 0), 0)})`}
                </button>
              </li>
              {categories.map((cat) => (
                <li key={cat.id}>
                  <button
                    onClick={() => updateParams({ category: category === cat.slug ? '' : cat.slug })}
                    className={`w-full rounded-lg px-3 py-1.5 text-left text-sm transition-colors ${
                      category === cat.slug ? 'bg-muted font-medium text-primary' : 'text-muted-foreground hover:bg-muted hover:text-foreground'
                    }`}
                  >
