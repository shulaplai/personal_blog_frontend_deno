  ...
  if (status === 'failed') {
    return (
      <div className="animate-fade-in py-16 text-center">
        ...
        <button onClick={() => dispatch(fetchPublicPostBySlug(slug))} className="text-primary underline hover:no-underline">
          Try again
        </button>
      </div>
    );
    ...
    <article className="animate-fade-in mx-auto py-8">
      ...
      {post.cover_image && (
        <div className="mb-8 overflow-hidden rounded-2xl">
          <img src={post.cover_image} alt={post.title} className="w-full max-h-96 object-cover" loading="lazy" />
        </div>
      )}
      ...
      <div className="rounded-2xl border border-border bg-background p-6 sm:p-8">
        <div
          className="prose-custom [&_img]:max-w-full [&_img]:rounded-xl [&_pre]:rounded-xl [&_pre]:border [&_pre]:border-border [&_pre]:bg-muted [&_pre]:p-4 [&_blockquote]:border-l-4 [&_blockquote]:border-primary [&_blockquote]:pl-4 [&_blockquote]:text-muted-foreground [&_h2]:mt-8 [&_h2]:mb-4 [&_h3]:mt-6 [&_h3]:mb-3 [&_p]:my-4 [&_p]:leading-relaxed [&_a]:text-primary [&_a]:underline"
        >
          {parse(post.content || '')}
        </div>
      </div>
