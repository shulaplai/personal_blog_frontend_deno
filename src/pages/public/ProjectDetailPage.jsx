  ...
  if (status === 'failed') {
    return (
      <div className="animate-fade-in py-16 text-center">
        ...
        <button
          onClick={() => dispatch(fetchPublicProjectBySlug(slug))}
          ...
          {project.technologies && project.technologies.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-1.5">
              {project.technologies.map((tech) => (
                <span
                  key={tech.id}
                  className="rounded-lg border border-border bg-muted px-2.5 py-0.5 text-xs text-muted-foreground"
                >
                  {tech.name}
                </span>
              ))}
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              {project.images.map((img) => (
                <div key={img.id}>
                  ...
                  />
                  {img.caption && (
                    <p className="mt-1 text-center text-xs text-muted-foreground">{img.caption}</p>
                  )}
                </div>
              ))}
            </div>
